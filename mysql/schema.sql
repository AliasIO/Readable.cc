...
CREATE TABLE IF NOT EXISTS `readable_cc`.`items_words` (
  ...
  INDEX `items_words_item_id_idx` (`item_id` ASC),
  INDEX `items_words_word_id_idx` (`word_id` ASC),
...
CREATE TABLE IF NOT EXISTS `readable_cc`.`users` (
  ...
  `external_links` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `readable_cc`.`users_feeds` (
  ...
  INDEX `users_feeds_user_id_idx` (`user_id` ASC),
  INDEX `users_feeds_feed_id_idx` (`feed_id` ASC),
  INDEX `users_feeds_folder_id_idx` (`folder_id` ASC),
